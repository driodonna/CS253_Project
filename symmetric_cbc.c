...
void encrypt(FILE *src, FILE *dest, unsigned char *key, unsigned char *iv){
    EVP_CIPHER_CTX ctx;
	...
	//how to set up the encryption (https://wiki.openssl.org/index.php/EVP_Symmetric_Encryption_and_Decryption)
  	if(1 != EVP_EncryptInit(&ctx,EVP_aes_128_cbc(),key,iv)) handleErrors();
...
void decrypt(FILE *src, FILE *dest, unsigned char *key, unsigned char *iv){
    EVP_CIPHER_CTX ctx;
	int destSize1 = 0, destSize2 = 0, srcSize;
    unsigned char *srcData, *destData;
	...
	//how to set up the encryption (https://wiki.openssl.org/index.php/EVP_Symmetric_Encryption_and_Decryption)
	if(1 != EVP_DecryptInit(&ctx,EVP_aes_128_cbc(),key,iv)) handleErrors();
...
int main(int argc, char *argv[]){        
    ...
    unsigned char *iv = "543210987654321";    
	FILE *src, *dest;
	
    src = fopen("lena512color.tiff", "rb");	//encrypt the lena image
    dest = fopen("cbc_ciphertext.txt", "wb"); //output the ciphertext
    encrypt(src, dest, key, iv); //encrypt function
    fclose(src);
    fclose(dest);

    src = fopen("cbc_ciphertext.txt", "rb"); //read the ciphertext
    dest = fopen("cbc_decrypted.tiff", "wb"); //decrypt the ciphertext
    decrypt(src,dest, key, iv);
    fclose(src);
    fclose(dest);
